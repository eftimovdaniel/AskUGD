...
def main() -> int:
    ...
    ap.add_argument("--dry-run", action="store_true", help="scrape + parche без пишување во Qdrant")
    ...
    logger.info("Вкупно %d парчиња од %d извори во '%s' (hybrid=%s)%s", statistika.chunks, statistika.sources_ok, settings.qdrant_collection,settings.use_hybrid, " [dry-run]" if argumenti.dry_run else "")
