...
class Status:
    ...
    def ok(seld, n_chunks:int)->None:
        self.chunks += n_chunks
        self.sources_ok += 1
    
    def fail (self, source: str, respon: str)-> None:
...
def main() -> int:
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="scrape + chunk без пишување во Qdrant")
    ...
    logger.info("Вкупно %d парчиња од %d извори во '%s' (hybrid=%s)%s",
                stats.chunks, stats.sources_ok, settings.qdrant_collection,
                settings.use_hybrid, " [dry-run]" if args.dry_run else "")
    if stats.failures:
